...
public class RunasUser
{
    ...
    public RunResult RunCmdAsUser(string username, string password, string domain, string command, uint timeOut, int logonProvider, bool isAdmin )
    {
        var result = new RunResult();
        try
        {
            result = new RunasUser().RunAs(username, password, command, domain, timeOut, 2, logonProvider, null, true, isAdmin, false);
    ...
    private RunResult RunAs(string username, string password, string cmd, string domainName, uint processTimeout, int logonType, int createProcessFunction, string[] remote, bool forceUserProfileCreation, bool bypassUac, bool remoteImpersonation)
    /*
    ...
    */
    {
        ...
        string commandLine = cmd;//ParseCommonProcessesInCommandline(cmd);
        int logonProvider = LOGON32_PROVIDER_DEFAULT;
        ...
        if (processTimeout > 0)
        {
            ...
            runResult.ExitCode = unchecked((int)exitCode);
            runResult.Output = output.Trim(Environment.NewLine.ToCharArray());
...
public static class AccessToken
{
    ...
    public static bool SetTokenIntegrityLevel(IntPtr hToken, IntegrityLevel integrity)
    {
        ...
        if (!result)
            throw new RunasCsException("[admin-error] Failed to set the token's Integrity Level: " + integrity.ToString());
        else
            ret = true;
        return ret;
    }
